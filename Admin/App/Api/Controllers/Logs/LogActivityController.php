...
class LogActivityController {
	...
	public function __construct() {
		...
		$hook_controller->add_action_hook( 'wp_login', array( $this, 'tailwatch_login_alert' ), 10, 2 );
		$hook_controller->add_action_hook( 'wp_logout', array( $this, 'tailwatch_logout_alert' ), 10, 1 );
		$hook_controller->add_action_hook( 'wp_login_failed', array( $this, 'tailwatch_failed_alert' ), 10, 2 );
		$hook_controller->add_action_hook( 'user_register', array( $this, 'tailwatch_register_alert' ), 10, 1 );
		$hook_controller->add_action_hook( 'retrieve_password', array( $this, 'tailwatch_password_reset_request_alert' ), 10, 1 );
		$hook_controller->add_filter_hook( 'lostpassword_user_data', array( $this, 'tailwatch_password_reset_non_exist_user' ), 10, 1 );
		$hook_controller->add_action_hook( 'password_reset', array( $this, 'tailwatch_password_reset_success_alert' ), 10, 2 );
	}
	 ...
	 * }
	 */
	public function tailwatch_log_activity_is_enabled() {
	 ...
	 */
	public function tailwatch_logs_activity_push_notification( $field_name ) {
	 ...
	 * @return void
	 */
	public function tailwatch_send_logs_activity_notification( $new_one, $field_name, $record_id = 0 ) {
		if ( $this->tailwatch_logs_activity_push_notification( $field_name ) ) {
			$severity = 'info';
			...
			$push_notification = new PushNotificationController();
			$push_notification->tailwatch_trigger_notification( $severity, $title, $message, $meta_data );
		}
	}
	 ...
	 */
	public function tailwatch_record_logs_activity( $data, $get_user_id, $type_is = 'JSON' ) {
		...
		return $activity_log_data->tailwatch_send_log_report( $data, $option, $type_is, null, absint( $get_user_id ), $facets );
	}
	 ...
	 */
	public function tailwatch_user_data_is( $username = '' ) {
		return array(
	 ...
	 * @return void
	 */
	public function tailwatch_login_alert( $user_login, $user ) {
		...
		if ( isset( $get_feature['field_1']['options']['option']['selected'] ) &&
			...
			$new_one = array(
				...
				'type'      => 'normal',
				'user_data' => wp_json_encode( $this->tailwatch_user_data_is( $user_login ) ),
				'status'    => 'active',
			);
			...
			$record_id   = $this->tailwatch_record_logs_activity( $new_one, $get_user_id, 'login_alert' );

			if ( in_array( 'administrator', $user_roles, true ) ) {
				$this->tailwatch_send_logs_activity_notification( $new_one, 'field_1', $record_id );
			}
		}
	}
	 ...
	 * @return void
	 */
	public function tailwatch_logout_alert( $username ) {
		$get_feature = $this->get_features_options();
		...
		if ( isset( $get_feature['field_2']['options']['option']['selected'] ) &&
			...
			$new_one = array(
				...
				'type'      => 'normal',
				'user_data' => wp_json_encode( $this->tailwatch_user_data_is( $user_data->user_login ) ),
				'status'    => 'active',
			);

			$user_roles = isset( $user_data->roles ) ? $user_data->roles : array();

			$record_id = $this->tailwatch_record_logs_activity( $new_one, $get_user_id, 'logout_alert' );

			if ( in_array( 'administrator', $user_roles, true ) ) {
				$this->tailwatch_send_logs_activity_notification( $new_one, 'field_2', $record_id );
			}
		}
	}
	...
	private const BLOCK_ERROR_CODES = array(
		...
		'tailwatch_user_blocked',      // Admin on-demand user block (Pro).
	);
	 ...
	 * @return void
	 */
	public function tailwatch_failed_alert( $username, $error = null ) {
		...
		if ( isset( $get_feature['field_3']['options']['option']['selected'] ) &&
			...
			if ( $user ) {
				$new_one = array(
					...
					'type'      => 'normal',
					'user_data' => wp_json_encode( $this->tailwatch_user_data_is( $user->user_login ) ),
					'status'    => 'active',
				...
				$get_user_id = absint( $user->ID );
				$record_id   = $this->tailwatch_record_logs_activity( $new_one, $get_user_id, 'incorrect_password' );

				if ( in_array( 'administrator', $user_roles, true ) ) {
					$this->tailwatch_send_logs_activity_notification( $new_one, 'field_3', $record_id );
				}
			}
		}
		...
		if ( isset( $get_feature['field_4']['options']['option']['selected'] ) &&
			...
			if ( $is_non_existing_user ) {
				$new_one = array(
					...
					'type'      => 'normal',
					'user_data' => wp_json_encode( $this->tailwatch_user_data_is( $username ) ),
					'status'    => 'active',
				);

				$get_user_id = 0;
				$record_id = $this->tailwatch_record_logs_activity( $new_one, $get_user_id, 'failed_non_user' );

				$this->tailwatch_send_logs_activity_notification( $new_one, 'field_4', $record_id );
			}
		}
	}
	 ...
	 * @return void
	 */
	public function tailwatch_register_alert( $user_id ) {
		$get_feature = $this->get_features_options();
		...
		if ( isset( $get_feature['field_5']['options']['option']['selected'] ) &&
			...
			$new_one = array(
				...
				'type'      => 'normal',
				'user_data' => wp_json_encode( $this->tailwatch_user_data_is( $user_info->user_login ) ),
				'status'    => 'active',
			);

			$get_user_id = absint( $user_info->ID );
			$record_id = $this->tailwatch_record_logs_activity( $new_one, $get_user_id, 'user_register' );

			$this->tailwatch_send_logs_activity_notification( $new_one, 'field_5', $record_id );
		}
	}
	 ...
	 * @return void
	 */
	public function tailwatch_password_reset_request_alert( $user_login_or_email ) {
		$get_feature = $this->get_features_options();
		...
		if ( $user && isset( $get_feature['field_6']['options']['option']['selected'] ) &&
			...
			$get_user_id = absint( $user->ID );
			$record_id   = $this->tailwatch_record_logs_activity( $new_one, $get_user_id, 'password_request' );

			if ( in_array( 'administrator', $user_roles, true ) ) {
				$this->tailwatch_send_logs_activity_notification( $new_one, 'field_6', $record_id );
			}
		}
	}
	 ...
	 */
	public function tailwatch_password_reset_non_exist_user( $user_data ) {
		$get_feature = $this->get_features_options();
		...
		if ( isset( $get_feature['field_7']['options']['option']['selected'] ) &&
			...
			if ( is_wp_error( $user_data ) ) {
				...
				$new_one = array(
					...
					'type'      => 'normal',
					'user_data' => wp_json_encode( $this->tailwatch_user_data_is( $user_login_or_email ) ),
					'status'    => 'active',
				);

				$get_user_id = 0;
				$record_id = $this->tailwatch_record_logs_activity( $new_one, $get_user_id, 'non_user_pass_request' );

				$this->tailwatch_send_logs_activity_notification( $new_one, 'field_7', $record_id );
			}
		}
	 ...
	 * @return void
	 */
	public function tailwatch_password_reset_success_alert( $user, $new_password ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- Parameter required by WordPress hook signature.
		$get_feature = $this->get_features_options();
		...
		if ( isset( $get_feature['field_8']['options']['option']['selected'] ) &&
			...
			$new_one = array(
				...
				'type'      => 'normal',
				'user_data' => wp_json_encode( $this->tailwatch_user_data_is( isset( $user->user_login ) ? $user->user_login : '' ) ),
				'status'    => 'active',
			);
			...
			$get_user_id = absint( $user->ID );
			$record_id   = $this->tailwatch_record_logs_activity( $new_one, $get_user_id, 'reset_success' );

			if ( in_array( 'administrator', $user_roles, true ) ) {
				$this->tailwatch_send_logs_activity_notification( $new_one, 'field_8', $record_id );
			}
		}
	}
}
