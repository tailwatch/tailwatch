...
class SmtpTestController {
	...
	public function wptw_smtp_test_email( $post_data = '' ) {
		try {
			...
			if ( empty( $test_email ) || ! is_email( $test_email ) ) {
				Log::error(
					'Test email failed due to invalid or empty email address: ' . $test_email,
					array(
						'feature'  => 'email_smtp',
						'action'   => 'smtp_test_email_send_failed',
						'detail'   => 'Test email failed due to invalid or empty email address: ' . $test_email,
						'origin'   => 'system',
						'severity' => 'medium',
					)
				);
			...
			$settings    = $smtp_config->wptw_get_smtp_configure_settings();
			...
			// Get from_email and from_name based on provider
			switch ( $provider_key ) {
				case 'default':
			...
			$subject      = 'Tailwatch SMTP Configuration Test';

			$email_template = new EmailTemplate();
			$message        = $email_template->wptw_test_email_template(
				$site_name,
				$site_url,
				$admin_email,
				$current_date,
			...
			if ( $result ) {
				Log::info(
					'Test email sent successfully to ' . $test_email . ' using ' . $provider_name . ' provider',
					array(
						'feature' => 'email_smtp',
				...
				return array(
					...
					'message' => sprintf( __( 'Test email sent successfully to %s!', 'tailwatch' ), esc_html( $test_email ) ),
				);
			} else {
				global $phpmailer;
				$error = $phpmailer ? $phpmailer->ErrorInfo : 'Unknown error';

				Log::error(
					'Test email failed to send to ' . $test_email . ' using ' . $provider_name . ' provider. Error: ' . $error,
					array(
						'feature'  => 'email_smtp',
						'action'   => 'smtp_test_email_send_failed',
						'detail'   => 'Test email failed to send to ' . $test_email . ' using ' . $provider_name . ' provider. Error: ' . $error,
						'origin'   => 'system',
						'severity' => 'high',
					)
				);
				...
				return array(
					...
					'message' => sprintf( __( 'Test email failed: %s', 'tailwatch' ), esc_html( $error ) ),
				);
			}
		} catch ( \Throwable $e ) {
			Log::error(
				...
				array(
					'feature'  => 'email_smtp',
					'action'   => 'smtp_test_email_send_failed',
					'detail'   => 'Exception occurred during SMTP test email process: ' . $e->getMessage(),
					'origin'   => 'system',
					'severity' => 'high',
				)
			);
