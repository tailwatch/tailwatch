...
class TablesOptimizeController {
	...
	public function wptw_get_spam_comments( $number_interval ): array {
		$number_of_days = $this->optimizer_controller->wptw_optimize_maintain_data();
		return $this->optimizer_model->get_spam_comments( $number_interval, $number_of_days );
	}

	public function wptw_clean_all_spam_comments( array &$json_data, $number_interval ): void {
		$spam_comments  = $this->wptw_get_spam_comments( $number_interval );
		$total_count    = count( $spam_comments );
		$rows_processed = $json_data['spam_comments']['rows_processed'];
		$sum_rows       = $rows_processed + $total_count;

		$this->optimizer_controller->wptw_optimize_logs_records(
			0 === $total_count
		...
		if ( empty( $spam_comments ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Spam comments cleanup completed', 'OK' );
		}
	}

	public function wptw_get_trash_comments( $number_interval ): array {
		$number_of_days = $this->optimizer_controller->wptw_optimize_maintain_data();
		return $this->optimizer_model->get_trash_comments( $number_interval, $number_of_days );
	}

	public function wptw_clean_trash_comments( array &$json_data, $number_interval ): void {
		$trash_comments = $this->wptw_get_trash_comments( $number_interval );
		...
		if ( empty( $trash_comments ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Trashed comments cleanup completed', 'OK' );
		}
	}

	public function wptw_get_drafts_post( $number_interval ): array {
		$number_of_days = $this->optimizer_controller->wptw_optimize_maintain_data();
		return $this->optimizer_model->get_auto_drafts( $number_interval, $number_of_days );
	}

	public function wptw_clean_auto_drafts( array &$json_data, $number_interval ): void {
		$auto_drafts    = $this->wptw_get_drafts_post( $number_interval );
		...
		if ( empty( $auto_drafts ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Auto drafts cleanup completed', 'OK' );
		}
	}

	public function wptw_get_trash_posts( $number_interval ): array {
		$number_of_days = $this->optimizer_controller->wptw_optimize_maintain_data();
		return $this->optimizer_model->get_trashed_posts( $number_interval, $number_of_days );
	}

	public function wptw_clean_trash_posts( array &$json_data, $number_interval ): void {
		$trashed_posts  = $this->wptw_get_trash_posts( $number_interval );
		...
		if ( empty( $trashed_posts ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Trashed posts cleanup completed', 'OK' );
		}
	}

	public function wptw_get_orphaned_post( $number_interval ): array {
		$number_of_days = $this->optimizer_controller->wptw_optimize_maintain_data();
		return $this->optimizer_model->get_orphaned_post_meta( $number_interval, $number_of_days );
	}

	public function wptw_clean_orphaned_post( array &$json_data, $number_interval ): void {
		$orphaned_meta  = $this->wptw_get_orphaned_post( $number_interval );
		...
		if ( empty( $orphaned_meta ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Orphaned post meta cleanup completed', 'OK' );
		}
	}


	public function wptw_get_expired_transients( $number_interval ): array {
		return $this->optimizer_model->get_expired_transients( $number_interval );
	}

	public function wptw_clean_expired_transients( array &$json_data, $number_interval ): void {
		$expired_transients = $this->wptw_get_expired_transients( $number_interval );
		...
		if ( empty( $expired_transients ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Expired transients cleanup completed', 'OK' );
		}
	}

	public function wptw_get_trackback_pingbacks( $number_interval ): array {
		$number_of_days = $this->optimizer_controller->wptw_optimize_maintain_data();
		return $this->optimizer_model->get_trackback_pingbacks( $number_interval, $number_of_days );
	}

	public function wptw_clean_trackback_pingbacks( array &$json_data, $number_interval ): void {
		$trackbacks_pingbacks = $this->wptw_get_trackback_pingbacks( $number_interval );
		...
		if ( empty( $trackbacks_pingbacks ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Trackbacks/Pingbacks cleanup completed', 'OK' );
		}
	}

	public function wptw_get_logs( $option, $number_interval = null ): array {
		$number_of_days = $this->optimizer_controller->wptw_optimize_maintain_data();
		return $this->optimizer_model->get_logs_by_option( $option, $number_of_days, $number_interval );
	}

	public function wptw_clean_logs_activity( array &$json_data, $number_interval = null ): void {
		$logs_activity  = $this->wptw_get_logs( 'default_logs_activity', $number_interval );
		...
		if ( empty( $logs_activity ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Activity logs cleanup completed', 'OK' );
		}
	}

	public function wptw_clean_ajax_logs( array &$json_data, $number_interval = null ): void {
		$ajax_logs        = $this->wptw_get_logs( 'default_ajax_logs', $number_interval );
		$total_ajax_logs  = $this->wptw_get_logs( 'default_ajax_logs', null );
		...
		if ( $verify_total_counts === $total_rows_counts || empty( $ajax_logs ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Network Logs cleanup completed', 'OK' );
		}
	}

	public function wptw_clean_monitoring_logs( array &$json_data, $number_interval = null ): void {
		$monitoring_logs = $this->wptw_get_logs( 'default_monitoring_logs', $number_interval );
		...
		if ( empty( $monitoring_logs ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Error logs cleanup completed', 'OK' );
		}
	}

	public function wptw_clean_email_logs( array &$json_data, $number_interval = null ): void {
		$email_logs     = $this->wptw_get_logs( 'default_email_logs', $number_interval );
		...
		if ( empty( $email_logs ) ) {
			...
			$this->optimizer_controller->update_db_optimization_data( $json_data );
			$this->optimizer_controller->wptw_optimize_logs_records( 'Email logs cleanup completed', 'OK' );
		}
	}
}
