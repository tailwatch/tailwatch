...
class IpManagementController {
	...
	public function wptw_preview_block_page( $post_data ) {
		...
		// MobileAppController both wp_unslash $_POST['data']); a second unslash here
		// would strip legitimate JSON escapes (e.g. \n in multi-line messages).
		$body = array();
	...
	private function show_default_block_page( $block_data ) {
		...
		echo $this->build_block_page_html( $block_data, $this->wptw_get_block_page_design() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- composed from sanitized design values + esc_* in the builder.
		exit;
	}
