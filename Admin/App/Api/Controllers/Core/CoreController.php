...
class CoreController extends BaseController {
	...
	protected function wptw_get_feature_status() {
		return $this->wptw_core_update_feature_enable();
	}
	 ...
	 */
	public function wptw_core_update_feature_enable() {
		$feature_settings = PluginThemeController::wptw_plugin_theme_options();
	 ...
	 */
	public function wptw_get_core_scheduler_interval() {
		$feature_settings = PluginThemeController::wptw_plugin_theme_options();
	...
	private function install_core_version( $target_version, $action_type = 'core_update' ) {
		...
		// Construct download URL directly - this avoids transient corruption issues.
		...
		// Verify the download URL is accessible before downloading.
		...
		if ( 200 !== $response_code ) {
			return array(
				'success' => false,
				// translators: 1: HTTP status code, 2: WordPress version number.
				'message' => sprintf( __( 'Download URL returned HTTP %1$s. Version %2$s may not exist.', 'tailwatch' ), $response_code, $target_version ),
				'code'    => 404,
			);
		}
		...
		if ( is_wp_error( $temp_file ) ) {
			Log::error(
				'WordPress core installation failed: Download failed',
				array(
					'feature'        => 'core',
			...
			return array(
				'success' => false,
				// translators: %s is the error message.
				'message' => sprintf( __( 'Failed to download WordPress version: %s', 'tailwatch' ), $temp_file->get_error_message() ),
				'code'    => 500,
			);
		}

		// Create temporary directory for extraction inside the uploads dir.
		// wp_mkdir_p() is used instead of $wp_filesystem->mkdir() so the parent
		// `tailwatch-temp/` directory is created recursively on first run.
		$upload_dir = wp_get_upload_dir();
		$temp_dir   = trailingslashit( $upload_dir['basedir'] ) . 'tailwatch-temp/wordpress-install-' . $target_version;
		...
		if ( ! wp_mkdir_p( $temp_dir ) ) {
			...
			Log::error(
				'WordPress core installation failed: Failed to create temporary directory',
				array(
					'feature'        => 'core',
					'action'         => "{$action_type}_failed",
					'target_version' => $target_version,
			...
			return array(
				'success' => false,
				'message' => __( 'Failed to create temporary directory.', 'tailwatch' ),
				'code'    => 500,
			);
		}
		...
		if ( is_wp_error( $unzip_result ) ) {
			$wp_filesystem->delete( $temp_dir, true );
			Log::error(
				'WordPress core installation failed: Extraction failed',
				array(
					'feature'        => 'core',
					'action'         => "{$action_type}_failed",
					'target_version' => $target_version,
			...
			return array(
				'success' => false,
				// translators: %s is the error message.
				'message' => sprintf( __( 'Failed to extract the WordPress files: %s', 'tailwatch' ), $unzip_result->get_error_message() ),
				'code'    => 500,
			);
		}

		// Copy files to WordPress root (skip wp-config.php and wp-content).
		$extracted_folder = $temp_dir . '/wordpress';
		$skip_files       = array( 'wp-config.php', 'wp-content' );
		$this->copy_files_to_root( $extracted_folder, ABSPATH, $skip_files );
	 ...
	 */
	public function wptw_update_core( $post_data = null ) {
		global $wp_version;
		...
		try {
			...
			set_transient( 'wptw_history_tracking_core_update', true, 60 );
	 ...
	 * @return array Response data with message and code.
	 */
	public function wptw_rollback_update_core( $post_data ) {
		global $wp_version;
		...
		try {
			...
			// Set flag to prevent duplicate history from hooks.
			set_transient( 'wptw_history_tracking_core_rollback', true, 60 );
			...
			if ( ! $result['success'] ) {
				...
				delete_transient( 'wptw_history_tracking_core_rollback' );
	 ...
	 */
	public function wptw_get_core_versions() {
		try {
	 ...
	 */
	public function wptw_get_wordpress_details() {
		try {
	 ...
	 */
	public function wptw_get_core_updates() {
		try {
			// Get actual current version from disk for accuracy.
			$current_version = $this->get_disk_version();
			if ( null === $current_version ) {
	 ...
	 */
	public function wptw_check_core_compatibility( $post_data ) {
		global $wp_version;
	...
	/**
	 * Copy files from source to destination (helper for rollback).
	 *
	 * @param string $source      Source directory path.
	 * @param string $destination Destination directory path.
	 * @param array  $skip_files  Array of filenames to skip.
	 * @return void
	 */
	private function copy_files_to_root( $source, $destination, $skip_files = array() ) {
		global $wp_filesystem;

		$dir = $wp_filesystem->dirlist( $source );
		if ( ! $dir ) {
			return;
		}

		foreach ( $dir as $fileinfo ) {
			$src_file  = trailingslashit( $source ) . $fileinfo['name'];
			$dest_file = trailingslashit( $destination ) . $fileinfo['name'];

			if ( in_array( $fileinfo['name'], $skip_files, true ) ) {
				continue;
			}

			if ( 'f' === $fileinfo['type'] ) {
				$wp_filesystem->copy( $src_file, $dest_file, true );
			} elseif ( 'd' === $fileinfo['type'] ) {
				if ( ! $wp_filesystem->is_dir( $dest_file ) ) {
					$wp_filesystem->mkdir( $dest_file );
				}
				$this->copy_files_to_root( $src_file, $dest_file, $skip_files );
			}
		}
	}
}
