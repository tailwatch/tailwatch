...
class ResetAllFeatureController {
	...
	public function __construct() {
		...
		$hook_controller->add_action_hook( 'tailwatch_reset_all_settings_batch_cron', array( $this, 'tailwatch_reset_all_settings_batch_cron' ) );
	}
	...
	private function register_process_monitoring() {
		ProcessManager::register_process(
			array(
				...
				'cron_hooks'         => array( 'tailwatch_reset_all_settings_batch_cron' ),
				'stuck_threshold'    => 300,
				'max_retries'        => 3,
	...
	public function tailwatch_start_reset_all_settings( $post_data = null ) {
		try {
			...
			$DBModel = new DBModel();
			$DBModel->delete_all_transients( '_transient_tailwatch_reset_' );
			...
			$reset_id = 'tailwatch_reset_' . bin2hex( random_bytes( 13 ) );

			$process_id             = $this->processManager->get_or_create_process(
				'reset_all',
				'tailwatch_reset_all_settings_batch_cron',
				array(
					'reset_id' => $reset_id,
					'total'    => $total_steps,
				)
			...
			if ( ! wp_next_scheduled( 'tailwatch_reset_all_settings_batch_cron', array( $reset_id ) ) ) {
				$schedule_result = wp_schedule_single_event( time() + 5, 'tailwatch_reset_all_settings_batch_cron', array( $reset_id ) );
				if ( ! $schedule_result ) {
					Log::error(
	...
	public function tailwatch_reset_all_settings_batch_cron( $reset_id ) {
		try {
			$batch_size  = 5;
			...
			if ( $reset_settings && empty( $batch_state['settings_done'] ) ) {
				...
				for ( $i = $current_index; $i < $total && $processed < $batch_size; $i++, $processed++ ) {
					...
					do_action( 'tailwatch_apply_plan_features_to_db', $option );
				}
				...
				if ( empty( $batch_state['settings_done'] ) ) {
					$this->tailwatch_reschedule_batch( $reset_id, $batch_state ); // more settings next tick
					return;
				}
			}
			...
			if ( $reset_data && empty( $batch_state['data_done'] ) ) {
				...
				while ( $di < count( $data_steps ) ) {
					$step_complete = $this->tailwatch_run_data_reset_step( $data_steps[ $di ], $deadline );
					if ( ! $step_complete ) {
				...
				if ( empty( $batch_state['data_done'] ) ) {
					$this->tailwatch_reschedule_batch( $reset_id, $batch_state ); // more data next tick
					return;
				}
			}
			...
			if ( $reset_settings ) {
				...
				// canonical hook list (pro's included via tailwatch_register_cron_hooks) and does NOT
				// touch this reset's cron or the verify cron, so it is safe to run mid-completion.
				CronJobManager::clear_all_cron_events();
				delete_option( 'tailwatch_created_cron_jobs' );
				delete_option( 'tailwatch_paused_cron_jobs' );
				delete_option( 'tailwatch_custom_schedules' );

				$security_controller = new SecurityFeaturesVerifyController();
				$security_controller->tailwatch_start_security_features_process();
			}
	 ...
	 */
	private function tailwatch_reschedule_batch( $reset_id, $batch_state ) {
		$batch_state['cron_running']         = true;
		$batch_state['completion_timestamp'] = time();
		$batch_state['function_completed']   = true;
		$batch_state['function_started']     = false;
		set_transient( $reset_id, $batch_state, 60 * 60 );
		wp_schedule_single_event( time() + 5, 'tailwatch_reset_all_settings_batch_cron', array( $reset_id ) );
	}
	 ...
	 */
	private function tailwatch_run_data_reset_step( $step, $deadline ) {
		global $wpdb;
		switch ( $step ) {
			case 'truncate_logs':
				...
				$wpdb->query( $wpdb->prepare( 'TRUNCATE TABLE %i', $wpdb->prefix . TAILWATCH_DB_LOGS_TABLE_NAME ) );
				return true;
			case 'truncate_baseline':
				...
				$wpdb->query( $wpdb->prepare( 'TRUNCATE TABLE %i', $wpdb->prefix . TAILWATCH_DB_FILEMON_BASELINE_TABLE ) );
				return true;
			case 'truncate_scans':
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- reset: fixed table-name constant.
				$wpdb->query( $wpdb->prepare( 'TRUNCATE TABLE %i', $wpdb->prefix . TAILWATCH_DB_FILEMON_SCANS_TABLE ) );
				return true;
			...
			case 'reset_pro_data':
				...
				do_action( 'tailwatch_reset_all_data' );
				return true;
			default:
				return true;
		}
	 ...
	 */
	private function tailwatch_purge_dir_contents( $dir, $deadline ) {
		if ( ! is_dir( $dir ) ) {
			return true;
		}
		...
		foreach ( $items as $item ) {
			...
			if ( is_dir( $path ) ) {
				if ( ! $this->tailwatch_purge_dir_contents( $path, $deadline ) ) {
					return false;
				}
				@rmdir( $path );
			} else {
	...
	public function tailwatch_reset_all_settings_status( $post_data = null ) {
		try {
			$reset_id = null;
			if ( $post_data ) {
			...
			// reset_ids we mint are `'tailwatch_reset_' . uniqid()`; reject
			// other shapes before the dynamic `get_transient()`.
			if ( ! empty( $reset_id ) && ! ( is_string( $reset_id ) && preg_match( '/^tailwatch_reset_[a-f0-9]{26}$/', $reset_id ) ) ) {
				return array(
					'code'    => 400,
	...
	public function tailwatch_reset_cron_if_failed( $post_data ) {
		try {
			...
			if ( $cron_running === false ) {
				if ( ! wp_next_scheduled( 'tailwatch_reset_all_settings_batch_cron', array( $reset_id ) ) ) {
					$cron_scheduled = wp_schedule_single_event( time() + 5, 'tailwatch_reset_all_settings_batch_cron', array( $reset_id ) );
					if ( $cron_scheduled ) {
						Log::info(
