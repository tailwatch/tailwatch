...
class AuthenticationController {
	...
	public function __construct() {
		...
		$hook_controller->add_filter_hook( 'authenticate', array( $this, 'tailwatch_handle_authentication' ), 25, 3 );
		$hook_controller->add_action_hook( 'wp_login_failed', array( $this, 'tailwatch_handle_failed_login' ), 10, 2 );
		$hook_controller->add_action_hook( 'wp_login', array( $this, 'tailwatch_handle_successful_login' ), 10, 2 );
		$hook_controller->add_action_hook( 'admin_init', array( $this, 'tailwatch_check_access' ), 10 );
		$hook_controller->add_filter_hook( 'login_errors', array( $this, 'tailwatch_custom_login_error_message' ), 10 );
	}

	public function tailwatch_handle_authentication( $user, $username, $password ) {
	...
	public function tailwatch_handle_failed_login( $username, $error ) {
		$this->ip_protection->handle_failed_login( $username, $error );
	}

	public function tailwatch_handle_successful_login( $user_login, $user ) {
		$this->ip_protection->handle_successful_login( $user_login, $user );
	}

	public function tailwatch_check_access() {
		$this->ip_protection->check_access();
	}

	public function tailwatch_custom_login_error_message( $error ) {
