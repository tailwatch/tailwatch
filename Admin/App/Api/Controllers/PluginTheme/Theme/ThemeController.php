...
class ThemeController extends BaseController {
	...
	protected function tailwatch_get_feature_status() {
		...
		return $plugin_theme_controller->tailwatch_theme_update_feature_enable();
	}
	 ...
	 * }
	 */
	public function tailwatch_get_all_installed_themes( $post_data ) {
		try {
			$json_data = isset( $post_data ) ? wp_unslash( $post_data ) : '';
			$data      = json_decode( $json_data, true );
			...
			// Call service layer.
			$themes = ThemeManagerService::tailwatch_get_all_themes(
				array(
					'page'         => $page,
					'limit'        => $limit,
	...
	public function tailwatch_activate_theme( $post_data ) {
		...
		try {
			...
			$result = ThemeManagerService::tailwatch_activate_theme( $theme_slug );
	...
	public function tailwatch_delete_theme( $post_data ) {
		...
		try {
			...
			$result = ThemeManagerService::tailwatch_delete_theme( $theme_slug );
	 ...
	 * }
	 */
	public function tailwatch_get_theme_versions( $post_data ) {
		try {
			$json_data = isset( $post_data ) ? wp_unslash( $post_data ) : '';
			$data      = json_decode( $json_data, true );
	 ...
	 * }
	 */
	public function tailwatch_theme_details( $post_data ) {
		try {
			$json_data = isset( $post_data ) ? wp_unslash( $post_data ) : '';
			$data      = json_decode( $json_data, true );
			...
			$result  = $service->tailwatch_plugin_theme_details( $slug, 'theme' );
	...
	public function tailwatch_theme_rollback( $post_data ) {
		...
		try {
			...
			$service = new PluginThemeService();
			$result  = $service->tailwatch_rollback( 'theme', $slug, $version );
	 ...
	 */
	public function tailwatch_check_theme_compatibility( $post_data ) {
		$theme_slug = null;
