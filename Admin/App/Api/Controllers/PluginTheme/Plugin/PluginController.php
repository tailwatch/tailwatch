...
class PluginController extends BaseController {
	...
	protected function tailwatch_get_feature_status() {
		...
		return $plugin_theme_controller->tailwatch_plugin_update_feature_enable();
	}
	...
	public function tailwatch_update_plugin( $post_data ) {
		...
		try {
			...
			set_transient( 'tailwatch_history_tracking_in_progress', true, 30 );
	 ...
	 * }
	 */
	public function tailwatch_get_all_installed_plugins( $post_data ) {
		try {
			$json_data = isset( $post_data ) ? wp_unslash( $post_data ) : '';
			$data      = json_decode( $json_data, true );
			...
			// Call service layer.
			$plugins = PluginManagerService::tailwatch_get_all_plugins(
				array(
					'page'              => $page,
					'limit'             => $limit,
	...
	public function tailwatch_activate_plugin( $post_data ) {
		...
		try {
			...
			$result = PluginManagerService::tailwatch_activate_plugins( array( $plugin ), $network_wide );
	...
	public function tailwatch_deactivate_plugin( $post_data ) {
		...
		try {
			...
			$result = PluginManagerService::tailwatch_deactivate_plugins( array( $plugin ), $network_wide );
	...
	public function tailwatch_delete_plugin( $post_data ) {
		...
		try {
			...
			$result = PluginManagerService::tailwatch_delete_plugins( array( $plugin ) );
	 ...
	 * }
	 */
	public function tailwatch_bulk_plugin_action( $post_data ) {
		$action        = null;
		$plugins_count = 0;
		...
		try {
			...
			switch ( $action ) {
				case 'activate':
					$result = PluginManagerService::tailwatch_activate_plugins( $valid_plugins, $network_wide );
					break;
				case 'deactivate':
					$result = PluginManagerService::tailwatch_deactivate_plugins( $valid_plugins, $network_wide );
					break;
				case 'delete':
					$result = PluginManagerService::tailwatch_delete_plugins( $valid_plugins );
					break;
			}
	 ...
	 * }
	 */
	public function tailwatch_get_plugin_versions( $post_data ) {
		try {
			$json_data = isset( $post_data ) ? wp_unslash( $post_data ) : '';
			$data      = json_decode( $json_data, true );
	 ...
	 * }
	 */
	public function tailwatch_plugin_details( $post_data ) {
		try {
			$json_data = isset( $post_data ) ? wp_unslash( $post_data ) : '';
			$data      = json_decode( $json_data, true );
			...
			// wp.org directory slug. tailwatch_plugin_theme_details() needs the directory slug.
			...
			$result  = $service->tailwatch_plugin_theme_details( $slug, 'plugin' );
	...
	public function tailwatch_plugin_rollback( $post_data ) {
		...
		try {
			...
			$service = new PluginThemeService();
			$result  = $service->tailwatch_rollback( 'plugin', $slug, $version );
	 ...
	 */
	public function tailwatch_check_plugin_compatibility( $post_data ) {
		$plugin_slug = null;
