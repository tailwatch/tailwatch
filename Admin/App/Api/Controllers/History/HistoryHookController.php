...
// phpcs:disable WordPress.WP.AlternativeFunctions.file_system_operations_glob -- Every glob() in this file scans a plugin/theme directory for its main *.php entry file; the wildcard pattern is what makes glob() the right tool here.
...
class HistoryHookController {
	...
	private function is_action_from_our_plugin() {
		...
		$flag_generic       = get_transient( 'tailwatch_history_tracking_in_progress' );
		$flag_core_update   = get_transient( 'tailwatch_history_tracking_core_update' );
		$flag_core_rollback = get_transient( 'tailwatch_history_tracking_core_rollback' );
	...
	public function track_core_update( $new_version ) {
		...
		if ( $this->is_action_from_our_plugin() ) {
			...
			delete_transient( 'tailwatch_history_tracking_in_progress' );
