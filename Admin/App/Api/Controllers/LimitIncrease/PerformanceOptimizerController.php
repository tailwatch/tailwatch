...
class PerformanceOptimizerController {
	...
	private $option = 'wptw_performance';
	 ...
	 * @var string
	 */
	private const HAS_SETTINGS_FLAG = 'wptw_has_php_performance';
	 ...
	 * @return array<string, mixed>
	 */
	public function wptw_get_php_settings() {
		...
		return array(
			...
			'diagnostic'            => $this->wptw_diagnose_php_limits(),
		);
	}
	 ...
	 * @return array<string, mixed>
	 */
	public function wptw_save_php_settings( $post_data ) {
		...
		if ( $result ) {
			...
			// Remove any legacy WPTW php_value block left by older versions —
	 ...
	 * @return array<string, mixed>
	 */
	public function wptw_remove_php_settings() {
		$db_model = new DBModel();
		...
		if ( $result ) {
			...
			delete_transient( 'wptw_htaccess_phpvalue_supported' );
			self::$cached_settings = null;
	...
	/**
	 * Removes any legacy WPTW Performance `php_value` block from .htaccess.
	 *
	...
	private function strip_legacy_htaccess_block() {
		...
		if ( false === $htaccess_content || '' === $htaccess_content ) {
			return array(
				'code'    => 200,
				'message' => '',
			);
		...
		$marker_start = '# BEGIN WPTW Performance Settings';
		$marker_end   = '# END WPTW Performance Settings';
		$pattern      = '#\n*' . preg_quote( $marker_start, '#' ) . '.*?' . preg_quote( $marker_end, '#' ) . '\n*#s';

		$stripped    = preg_replace( $pattern, "\n", $htaccess_content );
		$new_content = is_string( $stripped ) ? $stripped : $htaccess_content;

		if ( $new_content !== $htaccess_content ) {
			$wp_filesystem->put_contents( $htaccess_path, $new_content );
		}
	...
	private function attempt_htaccess_write( $settings ) {
		...
		$supported = get_transient( 'wptw_htaccess_phpvalue_supported' );
		if ( false === $supported ) {
			$supported = $this->probe_htaccess_php_value() ? '1' : '0';
			set_transient( 'wptw_htaccess_phpvalue_supported', $supported, DAY_IN_SECONDS );
		}
		if ( '1' !== $supported ) {
			return false;
		}
		...
		$original = $exists ? (string) $wp_filesystem->get_contents( $htaccess_path ) : '';

		$marker_start = '# BEGIN WPTW Performance Settings';
		$marker_end   = '# END WPTW Performance Settings';
		$pattern      = '#\n*' . preg_quote( $marker_start, '#' ) . '.*?' . preg_quote( $marker_end, '#' ) . '\n*#s';
		$stripped     = preg_replace( $pattern, "\n", $original );
		$base         = is_string( $stripped ) ? $stripped : $original;

		$new_content = ltrim( rtrim( $base ) . "\n\n" . $this->generate_htaccess_perdir_block( $settings ) . "\n", "\n" );

		if ( ! $wp_filesystem->put_contents( $htaccess_path, $new_content ) ) {
			return false;
		}

		// Verify the live site still responds; roll back on any failure (covers
		// the rare case the docroot's AllowOverride differs from the probe dir).
		if ( ! $this->loopback_ok( home_url( '/' ) ) ) {
			$wp_filesystem->put_contents( $htaccess_path, $original );
			set_transient( 'wptw_htaccess_phpvalue_supported', '0', DAY_IN_SECONDS );
			return false;
		}
	...
	private function probe_htaccess_php_value() {
		...
		$dir_name = 'wptw-phpcheck-' . wp_generate_password( 12, false );
	 ...
	 * @return string
	 */
	private function generate_htaccess_perdir_block( $settings ) {
		$directives = '';
		foreach ( $settings as $key => $value ) {
			if ( null === $value || '' === $value ) {
				continue;
		...
		$block = "# BEGIN WPTW Performance Settings\n";
		foreach ( array( 'mod_php.c', 'mod_php7.c', 'mod_php5.c' ) as $module ) {
			$block .= "<IfModule {$module}>\n" . $directives . "</IfModule>\n";
		}
		$block .= '# END WPTW Performance Settings';

		return $block;
	}

	/**
	 * Whether our WPTW php_value block is currently present in .htaccess.
	 *
	 * @return bool
	 */
	private function htaccess_block_present() {
		...
		return false !== strpos( $content, '# BEGIN WPTW Performance Settings' );
	}

	/**
	 * Whether our WPTW block is currently present in .user.ini.
	 *
	 * @return bool
	 */
	...
	/**
	 * Writes (or removes) the WPTW block in .user.ini for PHP-FPM / FastCGI /
	 ...
	 *
	 */
	public function wptw_boost_for_scanning() {
	...
	private function build_manual_php_snippets( $targets, $user_ini_supported ) {
		...
		if ( ! $user_ini_supported && '0' !== get_transient( 'wptw_htaccess_phpvalue_supported' ) ) {
			$directives = '';
			foreach ( $targets as $key => $value ) {
				if ( null === $value || '' === $value ) {
					continue;
	 ...
	 * @return array<string, mixed>
	 */
	public function wptw_diagnose_php_limits() {
