...
import { getIntegrationData, updateIntegrationData, deleteIntegrationData } from './IntegrationTabServices/IntegrationTabServices'
import { FiCheckCircle, FiXCircle, FiDatabase, FiClock, FiKey, FiRefreshCw, FiEdit2, FiX, FiSave, FiExternalLink, FiUserPlus, FiLogIn, FiSettings, FiTrash2 } from 'react-icons/fi'
                ...
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
                            To use the <strong>IP Management</strong> module and enable GeoIP-based features (country blocking, IP geolocation, brute force protection), you must connect a valid MaxMind license key below.
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="relative bg-white border border-gray-200 rounded-xl overflow-hidden w-full">
                ...
                <div className="p-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        {/* Next Update Check */}
                        <div className="bg-gray-50 rounded-lg px-4 py-3">
                            <p className="text-xs text-gray-500 mb-0.5">Next Update Check</p>
                            <div className="flex items-center gap-1.5">
                                <FiClock className={`text-xs ${maxmindData?.next_update_check ? 'text-[#007980]' : 'text-gray-400'}`} />
                                <p className={`text-sm font-semibold ${maxmindData?.next_update_check ? 'text-gray-800' : 'text-gray-400'}`}>
                                    {maxmindData?.next_update_check || '- -'}
                                </p>
                            </div>
                        </div>
                        ...
                        {/* Update Scheduled */}
                        <div className="bg-gray-50 rounded-lg px-4 py-3">
                            <p className="text-xs text-gray-500 mb-0.5">Update Scheduled</p>
                            {maxmindData?.is_update_scheduled === undefined || maxmindData?.is_update_scheduled === null ? (
                                <p className="text-sm font-semibold text-gray-400">- -</p>
                            ) : (
                                <div className="flex items-center gap-1.5">
                                    <FiRefreshCw className={`text-xs ${maxmindData.is_update_scheduled ? 'text-[#007980]' : 'text-gray-400'}`} />
                                    <p className={`text-sm font-semibold ${maxmindData.is_update_scheduled ? 'text-[#007980]' : 'text-gray-400'}`}>
                                        {maxmindData.is_update_scheduled ? 'Yes' : 'No'}
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        {/* Retry Attempts */}
                        <div className="bg-gray-50 rounded-lg px-4 py-3">
                            <p className="text-xs text-gray-500 mb-0.5">Retry Attempts</p>
                            {maxmindData?.retry_attempts > 0 ? (
                                <p className={`text-sm font-semibold ${maxmindData.max_retry_reached ? 'text-red-500' : 'text-amber-500'}`}>
                                    {maxmindData.retry_attempts} {maxmindData.max_retry_reached ? '(Max reached)' : ''}
                                </p>
                            ) : (
                                <p className="text-sm font-semibold text-gray-400">- -</p>
                            )}
