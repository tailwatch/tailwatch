...
import { getSystemSettings,getPhpSettings,updatePhpSettings } from "./SystemSettingService/SystemSettingService";
import { Server, Database, HardDrive, Shield, Settings, Globe, FileText, XCircle, Edit, Save, Info } from "lucide-react";
...
const SystemSettings = () => {
  ...
  const handleMemoryFieldChange = (key, value) => {
    // Remove any non-numeric characters except M
    let cleanValue = value.replace(/[^0-9M]/g, '');
    
    // Extract numeric part
    let numericPart = cleanValue.replace(/M/g, '');
    
    // Allow empty field for editing, but prevent zero
    if (numericPart === '0') {
      return;
    }
    
    // If empty, store empty value temporarily
    if (numericPart === '') {
      setEditablePhpSettings(prev => ({
        ...prev,
        [key]: ''
    ...
    // Ensure M is always at the end for non-empty values
  ...
  const handlePhpSave = async () => {
    try {
      // Validate memory fields before saving
      ...
      if(res?.data?.data?.code === 200){
        ...
        toast.success("PHP Settings updated successfully");
        setIsPhpEditMode(false);
      } else {
        toast.error("Something went wrong, please try again");
      }
    } catch (error) {
      console.error("Error saving PHP settings:", error);
            ...
            <div className="bg-white rounded-lg shadow-md border border-gray-300 p-6">
              ...
              <div className="mt-5 flex items-start gap-2.5 rounded-lg border border-amber-200 bg-amber-50 px-3.5 py-2.5">
                ...
                <p className="text-xs leading-relaxed text-amber-800">
                  <span className="font-semibold">Note:</span> Some PHP settings cannot be changed automatically because they are controlled by your hosting provider or server configuration. If a value does not update after applying changes, please modify it through your hosting control panel or contact your hosting provider.
                </p>
              </div>
            </div>
